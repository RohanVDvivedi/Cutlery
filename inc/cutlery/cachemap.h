...
/*
	...
	LFU -> saturating counters, O(log(n)) access due to use of heap, and inability to evict highly used objects from past, slow to adapt to changing workloads
	priority based -> too much complexity, and no temporal locality used

	This is a very simple LRU cache implementation, which is also a directive on how this library can be used to make more complex datastructures

	UNLIKE OTHER MAP LIKE CUTLERY DATA STRUCTURES (hashmap and bst) INSERTS WILL FAIL ON DUPLICATE INSERTIONS
*/
...
// a free floating cchnode is the one, that is not referenced in any cachemap, i.e a node that can be inserted to any cachemap
