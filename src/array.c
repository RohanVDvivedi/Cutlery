...
{
	if(array_p->occupied_size > index)
	{
...
{
	if(array_p->total_size < array_p->occupied_size && array_p->data_p_p != NULL)
	{
		// you fuck made an error
		return ;	
	}
	else if(array_p->total_size == array_p->occupied_size)
	{
		// compute new_size
		unsigned long long int new_total_size = ( array_p->total_size * array_p->increment_factor ) + array_p->increment_offset;

		// taking precaution, i.e. if new_total_size is the same as previous then just increment by 1
		new_total_size = new_total_size == array_p->occupied_size ? new_total_size + 1 : new_total_size;
		...
		memcpy(new_data_p_p, array_p->data_p_p, array_p->occupied_size * sizeof(void**));
	...
	array_p->data_p_p[array_p->occupied_size++] = new_data_p;
}

void print_array(array* array_p, void (*print_element)(void*))
{
	printf("\narray:");
	printf("\n\toccupied size : %lld", array_p->occupied_size);
