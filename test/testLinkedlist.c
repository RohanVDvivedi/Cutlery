...
int main()
{
	...
	// check get next and prev functionality
	{
		...
		const ts* mid_data = find_equals_in_linkedlist(ll, &((ts){-1}), &test_comparator);
	...
	print_ts(find_equals_in_linkedlist(ll, &((ts){4}), &test_comparator));
	printf("\n");

	printf("Searching for structure with a = 1\n");
	print_ts(find_equals_in_linkedlist(ll, &((ts){1}), &test_comparator));
	printf("\n");

	printf("Searching for structure with a = 2\n");
	print_ts(find_equals_in_linkedlist(ll, &((ts){2}), &test_comparator));
	printf("\n");
	...
	print_ts(find_equals_in_linkedlist(ll, &((ts){6}), &test_comparator));
	printf("\n");

	printf("Searching for structure with a = 5\n");
	print_ts(find_equals_in_linkedlist(ll, &((ts){5}), &test_comparator));
	printf("\n\n");
	...
	print_ts_linkedlist(testInsertAllAfter);
		...
		bubble_sort_linkedlist(ll, &compare_by_a_increasing_comparator);
		print_ts_linkedlist(ll);
	#endif

	// radix sort linkedlist
	printf("after bubble sort (reverse) : \n");
	bubble_sort_linkedlist(ll, &compare_by_a_decreasing_comparator);
	print_ts_linkedlist(ll);
