...
int main()
{
	...
	const int test_memory_size = 20;

	char test_memory_src[test_memory_size + test_iter];
	char test_memory_dest[test_memory_size + test_iter];
	...
	for(int i = 0; i < test_iter; i++)
	{
		for(int j = 0; j < test_iter; j++)
		{
			...
			init_rand_memory(test_memory_dest + j, test_memory_size);

			printf("\nInitial : \n");
			printf("src:");
			print_memory(test_memory_src + i, test_memory_size);
			printf("dest:");
			print_memory(test_memory_dest + j, test_memory_size);

			memory_move(test_memory_dest + j, test_memory_src + i, test_memory_size);
